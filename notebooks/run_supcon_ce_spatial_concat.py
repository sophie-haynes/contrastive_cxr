#!/usr/bin/env python3
"""
Joint SupCon + CE training with spatial feature concatenation.

This script trains a Siamese network with TWO objectives:
1. SupCon loss: Learn contrastive representations from individual lungs
2. CE loss: Classify pairs using spatially concatenated features

Architecture:
- Each lung → Backbone → Spatial features (B, 128, 7, 7)
- SupCon: Uses individual lung embeddings (after pooling)
- CE: Uses concatenated spatial features (before pooling)
- Both losses share the same backbone → joint gradients

Key insight: SupCon exploits pair structure (one lung pathological, other normal),
while CE uses reconstructed feature representation for classification.
"""
import os
import sys
import csv
...
class JointSupConCEModel(nn.Module):
    ...
    def forward(self, x1, x2):
        """
        Full forward pass returning both SupCon embeddings and CE logits.

        Returns:
            emb1, emb2: Individual lung embeddings for SupCon
            logits: Classification logits for CE
        """
        emb1, emb2 = self.forward_supcon(x1, x2)
...
def train_joint_epoch(model, dataloader, supcon_criterion, ce_criterion, optimizer,
                      device, ce_weight=1.0):
    """
    Training loop for joint SupCon + CE

    Args:
        model: JointSupConCEModel
        dataloader: DataLoader with individual lung labels
        supcon_criterion: SupConLoss
        ce_criterion: CrossEntropyLoss
        optimizer: Optimizer
        device: Device
        ce_weight: Weight for CE loss (SupCon weight is 1.0)
    """
    model.train()
    total_loss = 0.0
    ...
    for batch in dataloader:
        ...
        # 1. SupCon loss on individual lungs
        all_embeddings = torch.cat([emb1, emb2], dim=0)  # (2*B, 128)
        all_individual_labels = torch.cat([label1, label2], dim=0)  # (2*B,)
        supcon_loss = supcon_criterion(all_embeddings, all_individual_labels)

        # 2. CE loss on pairs (using concatenated features)
        ce_loss = ce_criterion(logits, pair_labels)

        # 3. Combine losses
        loss = supcon_loss + ce_weight * ce_loss
    ...
    return avg_loss, avg_supcon_loss, avg_ce_loss, accuracy


def eval_joint_epoch(model, dataloader, supcon_criterion, ce_criterion, device, ce_weight=1.0):
    """
    Evaluation loop with comprehensive metrics

    Metrics tracked:
    - Joint loss (SupCon + CE)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            all_individual_labels = torch.cat([label1, label2], dim=0)
            supcon_loss = supcon_criterion(all_embeddings, all_individual_labels)
    ...
    # Calculate embedding quality metrics using INDIVIDUAL labels (for SupCon)
...
def eval_joint_epoch_with_viz(model, dataloader, supcon_criterion, ce_criterion, device,
                               epoch, testset, plt_path="plots/supcon_ce_spatial_concat",
                               ce_weight=1.0):
    """Evaluation with visualization"""
    metrics = eval_joint_epoch(model, dataloader, supcon_criterion, ce_criterion, device, ce_weight)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            emb1, emb2 = model.forward_supcon(img1, img2)
    ...
    helpers.viz.plot_tsne_from_embeddings(
        ...
        all_labels,
        epoch=epoch,
        save_path=os.path.join(plt_path, f'{testset}_tsne_supcon_ce_epoch_{epoch}.png')
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train with joint SupCon + CE using spatial feature concatenation"
    )
    ...
    parser.add_argument("--ce_weight", type=float, default=1.0,
                       help="Weight for CE loss (SupCon weight is 1.0)")

    parser.add_argument("--lr", type=float, default=0.001)
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--workers", type=int, default=2)
    parser.add_argument("--plot_freq", type=int, default=5)
    parser.add_argument("--plot_dir", type=str, default="plots/supcon_ce_spatial_concat")
    parser.add_argument("--log_dir", type=str, default="logs/supcon_ce_spatial_concat")
...
def main():
    ...
    model = JointSupConCEModel(siamese_model, num_classes=2).to(device)
    ...
    # Log configuration
    config_str = (f"Training with Joint SupCon + CE (Spatial Concatenation):\n"
    ...
    # Setup CSV logging
    csv_cols = ['avg_loss', 'supcon_loss', 'ce_loss', 'accuracy',
    ...
    for epoch in range(start_epoch, args.epochs):
        # Training
        avg_loss, avg_supcon, avg_ce, train_accuracy = train_joint_epoch(
            model, train_dataloader, supcon_criterion, ce_criterion,
        ...
        if epoch % args.plot_freq == 0 and not args.no_tsne:
            train_metrics = eval_joint_epoch_with_viz(
                model, train_dataloader, supcon_criterion, ce_criterion, device,
                epoch, f"train-{args.train_source}", plot_dir, args.ce_weight
            )
            eval_metrics = eval_joint_epoch_with_viz(
                model, test_dataloader, supcon_criterion, ce_criterion, device,
                epoch, f"test-{args.train_source}", plot_dir, args.ce_weight
            )
            eval_metrics2 = eval_joint_epoch_with_viz(
                model, test2_dataloader, supcon_criterion, ce_criterion, device,
                epoch, f"test-{external_test_names[0]}", plot_dir, args.ce_weight
            )
            eval_metrics3 = eval_joint_epoch_with_viz(
                model, test3_dataloader, supcon_criterion, ce_criterion, device,
                epoch, f"test-{external_test_names[1]}", plot_dir, args.ce_weight
            )
        else:
            train_metrics = eval_joint_epoch(
                model, train_dataloader, supcon_criterion, ce_criterion, device, args.ce_weight
            )
            eval_metrics = eval_joint_epoch(
                model, test_dataloader, supcon_criterion, ce_criterion, device, args.ce_weight
            )
            eval_metrics2 = eval_joint_epoch(
                model, test2_dataloader, supcon_criterion, ce_criterion, device, args.ce_weight
            )
            eval_metrics3 = eval_joint_epoch(
                model, test3_dataloader, supcon_criterion, ce_criterion, device, args.ce_weight
            )

        # Logging
        if not args.quiet:
            print(f"\nEpoch {epoch+1}/{args.epochs}")
            print(f"  Train: Loss={avg_loss:.4f} (SupCon={avg_supcon:.4f}, CE={avg_ce:.4f}), Acc={train_accuracy:.2f}%")
