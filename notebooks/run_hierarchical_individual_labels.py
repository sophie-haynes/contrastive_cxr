#!/usr/bin/env python3
"""
Hierarchical loss training with individual lung labels.

This script trains a Siamese network using hierarchical loss (contrastive pair + SupCon)
where:
- Contrastive pair loss uses pair-level labels (normal vs nodule pair)
- SupCon loss uses individual lung labels (each lung gets its true label)

Key features:
- Uses ImagePairDatasetWithIndividualLabels
- Normal pairs: both lungs labeled as normal (0), pair label = 0
- Nodule pairs: one lung labeled nodule (1), other labeled normal (0), pair label = 1
- Contrastive pair loss: pulls normal pairs together, pushes nodule pairs apart
- SupCon loss: clusters lungs by their actual pathology (individual labels)
"""
import os
import sys
import csv
...
def train_hierarchical_epoch(model, dataloader, criterion, optimizer, device, use_proj=False,
                             use_ce_loss=False, ce_weight=1.0):
    """
    Training loop for Hierarchical loss with individual lung labels
    ...
    Args:
        ...
        use_ce_loss: If True, add cross-entropy classification loss
    ...
    # CE loss criterion
    ce_criterion = torch.nn.CrossEntropyLoss() if use_ce_loss else None
    ...
    for batch in dataloader:
        ...
        # Hierarchical loss with individual labels for SupCon
        # pair_class_idx: pair-level labels for contrastive pair loss
        # label1, label2: individual lung labels for SupCon loss
        ...
        # Add CE loss if enabled
        if use_ce_loss:
            # Get logits from classification head (on embeddings, not projections)
    ...
    avg_ce_loss = total_ce_loss / max(len(dataloader), 1) if use_ce_loss else 0.0

    return avg_loss, avg_pair_loss, avg_supcon_loss, avg_ce_loss


def eval_hierarchical_epoch(model, dataloader, criterion, device, use_proj=False,
                            use_ce_loss=False, ce_weight=1.0):
    """
    Evaluation loop with individual lung labels

    Metrics tracked:
    - Hierarchical loss (pair + supcon with individual labels + optional CE)
    - Pair loss component
    - SupCon loss component
    - CE loss component (if enabled) and accuracy
...
def eval_hierarchical_epoch_with_viz(model, dataloader, criterion, device, epoch, testset,
                                     plt_path="plots/hierarchical_individual", use_proj=False,
                                     use_ce_loss=False, ce_weight=1.0):
    """Evaluation with visualization"""
    metrics = eval_hierarchical_epoch(model, dataloader, criterion, device, use_proj,
                                      use_ce_loss, ce_weight)
    ...
    helpers.viz.plot_tsne_from_embeddings(
        ...
        all_labels,
        epoch=epoch,
        save_path=os.path.join(plt_path, f'{testset}_tsne_individual_epoch_{epoch}.png')
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train with Hierarchical loss (pair + SupCon) using individual lung labels"
    )
    ...
    parser.add_argument("--aggressive_aug", action="store_true",
                       help="Use official SupContrast augmentations (RandomResizedCrop, stronger ColorJitter)")
    ...
    parser.add_argument("--margin", type=float, default=1.0,
                       help="Margin for contrastive pair loss")
    ...
    parser.add_argument("--use_ce_loss", action="store_true",
                       help="Add cross-entropy classification loss on individual lung labels")
    parser.add_argument("--ce_weight", type=float, default=1.0,
                       help="Weight for CE loss (pair+supcon total weight is alpha+beta)")
    ...
    parser.add_argument("--plot_dir", type=str, default="plots/hierarchical_individual")
    parser.add_argument("--log_dir", type=str, default="logs/hierarchical_individual")
...
def main():
    ...
    # Loss function (Hierarchical: contrastive pair + SupCon with individual labels)
    criterion = HierarchicalLoss(
        alpha=args.alpha,
        beta=args.beta,
        margin=args.margin,
    ...
    exp_name_parts = [
        ...
        f"bsz{args.bsz}",
        f"lr{args.lr}",
        f"a{args.alpha}b{args.beta}",  # alpha and beta weights
        f"m{args.margin}",
    ...
    # Add "ce" to name if using cross-entropy loss
    if args.use_ce_loss:
        exp_name_parts.insert(1, f"ce{args.ce_weight}")  # Insert after model_name (and proj/spatial/early if present)

    # Add "aggaug" to name if using aggressive augmentation
    if args.aggressive_aug:
        exp_name_parts.append("aggaug")
    ...
    # Log configuration
    config_str = (f"Training with Hierarchical Loss (Pair + SupCon) + Individual Lung Labels:\n"
                 ...
                 f"  use_ce_loss={args.use_ce_loss}, ce_weight={args.ce_weight}\n"
                 f"  use_proj={args.use_proj}, projection_dim={args.projection_dim}\n"
                 f"  symmetrical_transforms={args.symmetrical_transforms}\n"
    ...
    # Setup CSV logging
    csv_cols = ['avg_loss', 'pair_loss', 'supcon_loss', 'ce_loss', 'accuracy',
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not args.quiet:
            ...
            if args.use_ce_loss:
                print(f"  Train: Loss={avg_loss:.4f}, Pair={avg_pair_loss:.4f}, SupCon={avg_supcon_loss:.4f}, CE={avg_ce_loss:.4f}")
            ...
            postfix_dict = {
                ...
                'Pair': f'{avg_pair_loss:.4f}',
                'SupCon': f'{avg_supcon_loss:.4f}',
                'Test_Sil': f'{eval_metrics["silhouette_score"]:.4f}'
            }
            if args.use_ce_loss:
                postfix_dict['CE'] = f'{avg_ce_loss:.4f}'
                postfix_dict['Test_Acc'] = f'{eval_metrics["accuracy"]:.2f}%'
        ...
        # Also save best accuracy model if using CE loss
        if args.use_ce_loss:
            is_best_accuracy = eval_metrics['accuracy'] > best_accuracy
            if is_best_accuracy:
                best_accuracy = eval_metrics['accuracy']
                # Save as best_model_accuracy.pth
                best_acc_path = os.path.join(log_dir, "checkpoints", 'best_model_accuracy.pth')
                checkpoint = {
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                ...
                print(f"New best accuracy model saved at epoch {epoch}! Accuracy: {best_accuracy:.2f}%")
