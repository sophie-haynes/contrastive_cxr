#!/usr/bin/env python3
"""
SupCon training with individual lung labels.

This script trains a Siamese network using SupCon loss where each lung gets its
true label (normal or nodule), rather than using the pair label for both lungs.

Key difference from run_supcon_experiment.py:
...
This allows SupCon to properly cluster lungs by their actual pathology.
"""
import os
import sys
import csv
...
# Import the SupCon loss
from helpers.supcon import SupConLoss
...
def train_supcon_individual_epoch(model, dataloader, criterion, optimizer, device, use_proj=False,
                                  use_ce_loss=False, ce_weight=1.0):
    """
    Training loop for SupCon with individual lung labels
    ...
    Args:
        ...
        ce_weight: Weight for CE loss (contrastive loss weight is 1.0)
    """
    model.train()
    total_loss = 0.0
    ...
    for batch in dataloader:
        ...
        # Get embeddings or projections for contrastive loss
        if use_proj:
        ...
        # SupCon: concatenate embeddings and their INDIVIDUAL labels
        all_embeddings = torch.cat([emb1, emb2], dim=0)
        all_labels = torch.cat([label1, label2], dim=0)

        contrastive_loss = criterion(all_embeddings, all_labels)
    ...
    avg_contrastive_loss = total_contrastive_loss / max(len(dataloader), 1)
    avg_ce_loss = total_ce_loss / max(len(dataloader), 1) if use_ce_loss else 0.0

    return avg_loss, avg_contrastive_loss, avg_ce_loss


def eval_supcon_individual_epoch(model, dataloader, criterion, device, use_proj=False,
                                 use_ce_loss=False, ce_weight=1.0):
    """
    Evaluation loop with individual lung labels

    Metrics tracked:
    - Loss (SupCon with individual labels + optional CE)
    ...
    - Pair distances by pair class (for compatibility with old metrics)
    """
    model.eval()
    total_loss = 0.0
    total_contrastive_loss = 0.0
    ...
    # Track pair distances by pair class (for compatibility)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Compute contrastive loss with individual labels (using projections if use_proj=True)
            all_proj = torch.cat([proj1, proj2], dim=0)
            all_labels = torch.cat([label1, label2], dim=0)
            contrastive_loss = criterion(all_proj, all_labels)
            total_contrastive_loss += contrastive_loss.item()
            ...
            # Calculate distances between pairs (for compatibility with old metrics)
            distances = F.pairwise_distance(emb1, emb2, p=2)

            # Separate by PAIR class (for compatibility)
...
def eval_supcon_individual_epoch_with_viz(model, dataloader, criterion, device, epoch, testset,
                                         plt_path="plots/individual_labels", use_proj=False,
                                         use_ce_loss=False, ce_weight=1.0):
    """Evaluation with visualization"""
    metrics = eval_supcon_individual_epoch(model, dataloader, criterion, device, use_proj,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train with SupCon loss using individual lung labels"
    )
    ...
    parser.add_argument("--ce_weight", type=float, default=1.0,
                       help="Weight for CE loss (contrastive loss weight is 1.0)")
    ...
    parser.add_argument("--plot_dir", type=str, default="plots/individual_labels")
    parser.add_argument("--log_dir", type=str, default="logs/individual_labels")
...
def main():
    ...
    # Determine num_classes for classification head (only if using CE loss)
    num_classes = 2 if args.use_ce_loss else None
    ...
    # Log configuration
    config_str = (f"Training with SupCon + Individual Lung Labels:\n"
    ...
    # Setup CSV logging
    csv_cols = ['avg_loss', 'contrastive_loss', 'ce_loss', 'accuracy',
    ...
    for epoch in range(start_epoch, args.epochs):
        # Training
        avg_loss, avg_contrastive_loss, avg_ce_loss = train_supcon_individual_epoch(
        ...
        if not args.quiet:
            ...
            if args.use_ce_loss:
                print(f"  Train: Loss={avg_loss:.4f}, Contrastive={avg_contrastive_loss:.4f}, CE={avg_ce_loss:.4f}")
