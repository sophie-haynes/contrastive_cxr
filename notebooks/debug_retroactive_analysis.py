...
def compute_attention_entropy(model, dataloader, device, max_batches=10):
    ...
    if not hasattr(model, 'get_attention_maps'):
        print("  Model doesn't have get_attention_maps method")
        return {'attention_entropy': None, 'normalized_entropy': None}
    
    if not hasattr(model, 'cross_attn_layers') or len(model.cross_attn_layers) == 0:
        print("  Model has no cross_attn_layers")
        return {'attention_entropy': None, 'normalized_entropy': None}
        ...
        for batch_idx, (img1, img2, labels, _, _) in enumerate(dataloader):
            ...
            try:
                ...
                if batch_idx == 0:
                    ...
                    print(f"  Contains NaN: {torch.isnan(attn_weights).any()}")
                
                B, H, W, _, _ = attn_weights.shape
                attn_flat = attn_weights.view(B, H*W, H*W)
                
                # Check for NaN or invalid values
                if torch.isnan(attn_flat).any() or torch.isinf(attn_flat).any():
                    print(f"  Warning: Attention weights contain NaN/Inf at batch {batch_idx}")
                    continue
                ...
                if torch.isnan(entropy).any() or torch.isinf(entropy).any():
                    print(f"  Warning: Entropy contains NaN/Inf at batch {batch_idx}")
                    continue
                    
                entropies.append(entropy.mean().cpu().item())
                
            except Exception as e:
                print(f"  Error computing attention entropy at batch {batch_idx}: {e}")
                import traceback
                traceback.print_exc()
                return {'attention_entropy': None, 'normalized_entropy': None}
    
    if entropies and H is not None and W is not None:
        avg_entropy = np.mean(entropies)
        max_entropy = np.log(H * W)
        normalized_entropy = avg_entropy / max_entropy
        
        print(f"  Computed entropy from {len(entropies)} batches")
    ...
    print(f"  No valid entropies computed (got {len(entropies)} values)")
    return {'attention_entropy': None, 'normalized_entropy': None}
