#!/usr/bin/env python3
"""
SupCon training with Prototypes and individual lung labels.

This script extends run_supcon_individual_labels.py by incorporating supervised
...
- SupConLossWithPrototypes with margin-based prototype pulling
...
# Import the SupCon loss with prototypes
from helpers.supcon import SupConLossWithPrototypes, compute_prototypes_from_embeddings
...
def train_supcon_proto_epoch(model, dataloader, criterion, optimizer, device, use_proj=False):
    """
    Training loop for SupCon with prototypes and individual lung labels

    Args:
        model: Siamese network model
        dataloader: DataLoader with individual lung labels
        criterion: SupConLossWithPrototypes
        optimizer: Optimizer
        device: Device
    ...
    for batch in dataloader:
        ...
        # SupCon with prototypes: concatenate embeddings and their INDIVIDUAL labels
...
def eval_supcon_proto_epoch(model, dataloader, criterion, device, use_proj=False):
    """
    Evaluation loop with individual lung labels and prototypes

    Metrics tracked:
    - Loss (SupCon with prototypes and individual labels)
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            all_labels = torch.cat([label1, label2], dim=0)
            loss = criterion(all_proj, all_labels)
            total_loss += loss.item()
...
def eval_supcon_proto_epoch_with_viz(model, dataloader, criterion, device, epoch, testset,
                                     plt_path="plots/proto_individual", use_proj=False):
    """Evaluation with visualization"""
    metrics = eval_supcon_proto_epoch(model, dataloader, criterion, device, use_proj)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train with SupCon + Prototypes using individual lung labels"
    )
    ...
    parser.add_argument("--plot_dir", type=str, default="plots/proto_individual")
    parser.add_argument("--log_dir", type=str, default="logs/proto_individual")
...
def main():
    ...
    # Loss function (SupCon with Prototypes)
    criterion = SupConLossWithPrototypes(
    ...
    # Log configuration
    config_str = (f"Training with SupCon + Prototypes + Individual Lung Labels:\n"
    ...
    # Setup CSV logging
    csv_cols = ['avg_loss', 'normal_pair_dist_mean', 'nodule_pair_dist_mean',
